    ...
    def update(self, found, cx, cy, r):
        if found:
            if self.sx is not None:
                ...
                self.lost = 0; self.ok = True
            self.sx = EMA_ALPHA * cx + (1 - EMA_ALPHA) * (self.sx or cx)
            self.sy = EMA_ALPHA * cy + (1 - EMA_ALPHA) * (self.sy or cy)
            self.sr = EMA_ALPHA * r  + (1 - EMA_ALPHA) * (self.sr or r)
            return True
        self.lost += 1
        if self.lost > LOST_FRAME_LIMIT: self.ok = False
        return self.ok
    def pos(self):
        return (int(self.sx or 0), int(self.sy or 0), int(self.sr or 0))
...
while True:
    ...
    if found:
        ...
        cv2.putText(frame, f"Lost: {tracker.lost_count}", (10, 75),
